...
const Research = () => {
  ...
  return (
    <section id="research" className="py-20 bg-white dark:bg-slate-800 relative overflow-hidden">
      {/* Animated Floating Background Elements - Made transparent */}
      <motion.div
          ...
          <div className="space-y-8">
            {researchData.map((paper, index) => (
              <motion.div
                ...
                className="bg-white/90 dark:bg-slate-700/90 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-600 cursor-pointer"
              ...
              >
                <div className="flex items-start gap-6">
                  ...
                  <motion.div
                    ...
                    transition={{ duration: 0.6 }}
                    className={`flex-shrink-0 w-20 h-20 bg-gradient-to-br ${paper.color} rounded-2xl flex items-center justify-center text-4xl shadow-lg`}
                  >
                    {paper.icon}
                  </motion.div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-3">
                      <h3 className="text-2xl font-bold text-slate-900 dark:text-white pr-4 leading-tight hover:text-blue-500 transition-colors">
                        {paper.title}
                      </h3>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {paper.keywords.slice(0, 5).map((keyword) => (
                        <span
                          key={keyword}
                          className="px-3 py-1 bg-slate-100 dark:bg-slate-600 text-slate-600 dark:text-slate-300 text-xs rounded-md font-medium"
                        >
                          {keyword}
                        </span>
                      ))}
                    ...
                    <div className="flex gap-3">
                      {paper.github && (
                        <motion.a
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-slate-900 dark:bg-slate-700 text-white rounded-lg hover:bg-slate-800 transition-colors text-sm font-semibold"
                        >
                          <Code2 size={16} />
                          Code
                        </motion.a>
                      ...
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        onClick={() => setSelectedPaper(paper)}
                        className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-500 to-teal-500 text-white rounded-lg hover:from-green-600 hover:to-teal-600 transition-colors text-sm font-semibold"
                      >
                        <FileText size={16} />
                        Details
                      </motion.button>
      ...
      <AnimatePresence>
        {selectedPaper && (
          ...
          >
            ...
            >
              {/* Modal Header */}
              <div className={`sticky top-0 bg-gradient-to-br ${selectedPaper.color} p-8 z-10`}>
                <div className="flex items-start gap-4">
                  <span className="text-7xl">{selectedPaper.icon}</span>
                  <div className="flex-1">
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {Object.entries(selectedPaper.results).map(([key, value]) => (
                      <div key={key} className="p-4 bg-gradient-to-br from-blue-50 to-purple-50 dark:from-slate-700 dark:to-slate-600 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {selectedPaper.github && (
                    <motion.a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center gap-2 px-6 py-4 bg-slate-900 dark:bg-slate-700 text-white rounded-xl hover:bg-slate-800 transition-colors font-semibold"
                    >
                      <Code2 size={24} />
                      View Code
                    </motion.a>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={() => setSelectedPaper(null)}
                    className="flex items-center justify-center gap-2 px-6 py-4 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-300 rounded-xl hover:bg-slate-300 dark:hover:bg-slate-500 transition-colors font-semibold"
                  >
                    Close
                  </motion.button>
                </div>
